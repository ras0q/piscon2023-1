//nolint:ifshort,errcheck,revive
package main
...
func getNewCategoryItems(w http.ResponseWriter, r *http.Request) {
	...
	if itemID > 0 && createdAt > 0 {
		...
		inQuery, inArgs, err = sqlx.In(
			baseInQuery+"AND (i.`created_at` < ?  OR (i.`created_at` <= ? AND i.`id` < ?)) ORDER BY i.`created_at` DESC, i.`id` DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			time.Unix(createdAt, 0),
		...
		// 1st page
		inQuery, inArgs, err = sqlx.In(
			baseInQuery+"ORDER BY i.`created_at` DESC, i.id DESC LIMIT ?",
			ItemStatusOnSale,
			ItemStatusSoldOut,
			categoryIDs,
			ItemsPerPage+1,
